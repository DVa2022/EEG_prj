...
class PathFinder:
    def __init__(self, filtered, tbl):
        ...
        if filtered:
            ...
            if 'C:\\Users\\atver' in os.getcwd():  # is my cpu
                self.DB_path = "C:/Users/atver/mne_data/physionet-sleep-data/"  # getting data from filtered data
            elif 'David' in os.getcwd():
...
class EEGDataset(Dataset):
    ...
    def __init__(self, Transform, exp_table):
        ...
        self.minmax = pd.read_csv(os.getcwd() + "/mins_maxs.csv")
        try:
            # get seg_len
            seg_len = -1
            i = 0
    ...
    def __getitem__(self, index):
        ...
        meas_date = self.minmax['meas_date'][self.minmax['filename'] == rec_paths[0][-16:]].iloc[0]
        try:
        ...
        with contextlib.redirect_stdout(io.StringIO()) as f:  #  see if dropping contextlib speeds up
            raw = mne.io.read_raw_edf(rec_paths[0])
            raw.crop(rosh, sof)
            # meas_date = raw.info['meas_date'] # compare speed of raw info and pandas
            # meas_date = pd.to_datetime(self.minmax['meas_date'][self.minmax['filename'] == rec_paths[0][-16]])
            # pd.to_datetime(minmax['meas_date'][minmax['filename'] == "SC4022E0-PSG.edf"])
            ch = ['EEG Fpz-Cz', 'EEG Pz-Oz']
            raw.pick_channels(ch, ordered=True)
            raw.load_data()
            raw = raw.filter(l_freq=1, h_freq=49)  # avg speed was .0595 without with is .076
            data1 = raw[ch[0]][0]  # np.ndarray (1,signal length)
            # data1min = min1 = data1.min()
            data2 = raw[ch[1]][0]
            # data2min = min2 = data2.min()
            raw.close()
        # print(data1min)
        min0 = self.minmax['min0'][self.minmax['filename'] == rec_paths[0][-16:]].to_numpy()
        # print([self.minmax['filename'] == rec_paths[0][-16]])
...
def filter_save(overwrite):
    ...
    if 'stu' in os.getcwd():  # on triton
        newDBpath = '/home/stu16/physionet_data/'
    elif 'atver' in os.getcwd():
        newDBpath = 'C:/Users/atver/mne_data/physionet-sleep-data/'
    elif 'David' in os.getcwd():
        newDBpath = 'C:/Users/David/mne_data/physionet-sleep-data/'
    ...
    for rec in range(len(tbl)):
        ...
        # Normalize
        datal = raw[ch[0]][0]  # np.ndarray (1,signal length)
        datalmin = datal.min()
...
def forward_epoch(model, dl, loss_function, optimizer, total_loss=0,
                  to_train=False, desc=None, device=torch.device('cpu'), label=0, weighting=False, bce2=False):
    # label =0 is for sex
    # label = 1 is for Age
    ...
    with tqdm(total=len(dl), desc=desc, ncols=100) as pbar:
        ...
        for i_batch, (X, y) in enumerate(dl):
            ...
            # Loss:
            y_true = y.type(torch.double) # changed for ce_w # TODO
...
class my_weighted_bce():
    ...
    def __call__(self, y_pred: torch.tensor, y_true: torch.tensor):
        ...
        w_bce = torch.mean(-(self.w*(y_true*torch.clamp(torch.log(y_pred), min=-100, max=100))
                           + (1-y_true)*torch.clamp(torch.log(1-y_pred), min=-100, max=100)))
        return w_bce


# class EEGDataset_old(Dataset):
#
#     def __init__(self, Transform, exp_table, filtered=1):
#         super().__init__()  # When using a subclass, remember to inherit its properties.
#         # ------Your code------#
